package com.moz.ates.traffic.common.component.validate;
...
public class ValidateChecker {
    ...
    public ValidateResult isValid(Object value){
        ...
        for(ValidateChecker check : checker){
            String message =  MozatesCommonUtils.isNull(check.getFailMessage()) ? check.getDefaultFailMessage() : check.getFailMessage();
            Pattern pattern;
            Matcher matcher;
            
            switch (check.getRule()) {
                case REQUIRED:
                    result = MozatesCommonUtils.isNotEmpty(String.valueOf(value));
                    break;
                case MAXLENGTH:
